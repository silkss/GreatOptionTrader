...
internal class InstrumentToGroupDTOConverter : IMultiValueConverter {
    public object Convert (object[] values, Type targetType, object parameter, CultureInfo culture) {
        if (values[0] is not InstrumentGroup group)             return new();
        if (values[1] is not string name)             return new();
        if (values[2] is not string exchange)             return new();
        if (string.IsNullOrEmpty(name) || string.IsNullOrEmpty(exchange))             return new();
